...
import type { TableColumn, SortOrder, SortOrderFlag, ColumnKey } from '../types'
...
export function getColKey(col: TableColumn): ColumnKey {
  if (col.type === 'selection') return '__vuevctable_selection__'
  if (col.type === 'expand') return '__vuevctable_expand__'
  return col.key
}
