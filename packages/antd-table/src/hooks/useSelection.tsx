...
export default function useRowSelection<RecordType>(
  ...
  const [mergedSelectedKeys, setMergedSelectedKeys] = useMergedState(
    mergedRowSelection.value.selectedRowKeys || EMPTY_LIST,
    {
  ...
  const setSelectedKeys = (keys: Key[]) => {
    ...
    const { preserveSelectedRowKeys, onChange: onSelectionChange } =
  ...
  const mergedSelections = computed(() => {
    const { onSelectInvert, onSelectNone, selections, hideSelectAll } =
      mergedRowSelection.value
            ...
            setSelectedKeys(
              data.value
                ...
                .filter(key => {
                  ...
                  return (
                    !checkProps?.disabled ||
                    derivedSelectedKeySet.value.has(key)
                  )
                })
            )
          }
  ...
  const transformColumns = (
    ...
    if (selectionType !== 'radio') {
      ...
      const allDisabled =
        ...
        allDisabledData.length === flattedDataLength.value
      ...
      title = !hideSelectAll && (
        <div class={`${prefixCls.value}-selection`}>
          <Checkbox
            checked={
              !allDisabled
                ? !!flattedDataLength.value && checkedCurrentAll
    ...
    const renderSelectionCell: ColumnType<RecordType>['customRender'] = ({
      record,
      index
    }) => {
